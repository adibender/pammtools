...
make_newdata.ped <- function(x, ...) {
  ...
  # drop internal columns not needed for prediction
...
get_zvars <- function(func, time_var, n_func) {
  ...
  time_vars <- make_mat_names(
    c(time_var, func$tz_var, "LL"),
    func$latency_var,
    func$tz_var,
    func$suffix,
    n_func
