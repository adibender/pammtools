...
#' @import pammtools
#' @author Philipp Kopper
as_ped_cr <- function(data, formula, id, censor_code = 0L, 
                      output = c("list", "data.frame"), cut = NULL, ...) {
  ...
  output <- match.arg(output, c("list", "data.frame"))
...
#' It is the operative sub-routine of pam_cr / pem_cr.
#' @param formula an object of class formula or a string convertible to it.
