...
split_data <- function(
  ...
  if (length(missing_vars) > 0) {
    stop(paste(
      "Variables provided in formula not in data set:",
  ...
  if (any(proposed_names %in% names(data))) {
    stop(paste0(
      "Error in attempt to rename provided time/status variables:
      Variables ",
      intersect(proposed_names, names(data)),
      " allready in data set."
    ))
  }
  data <- rename(
    data,
  ...
  if (length(surv_vars) == 3) {
    split_df <- split_df %>%
      mutate(
        ped_status = ifelse(
          .data$ped_status == 1 & .data$ped_time > max(cut),
          0L,
          .data$ped_status
        ),
        ...
        offset = ifelse(
          .data$ped_time == .data$tstart,
          -Inf,
          log(.data$ped_time - .data$tstart)
        )
      ) %>%
  ...
  if (length(surv_vars) == 3) {
    info_cut <- split_df %>%
      select(one_of(c("tstart", "ped_time"))) %>%
      unique()
  } else {
    info_cut <- cut
  }
...
split_data_multistate <- function(
  ...
  if (timescale == "gap") {
    data <- mutate(
      data,
      !!!list(.time = quo(!!as.name(surv_vars[2]) - !!as.name(surv_vars[1])))
    )
    formula <- update(formula, Surv(.time, status) ~ .)
    formula <- update_formula(
      formula,
      proposed_names = c(".time", surv_vars[3])
    )
  }
  ...
  data_list <- data_list[
    map_dbl(data_list, ~ sum(.x[[surv_vars[3]]])) >= min_events
  ]
  cuts <- get_cut(
    data_list,
    formula,
