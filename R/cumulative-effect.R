...
get_cumu_eff <- function(data, model, term, z1, z2 = NULL, se_mult = 2) {
  ...
  coefs <- coefs[col_ind]
  Vp <- get_Vp(model)[col_ind, col_ind]
  X <- make_X(model, ped)[, col_ind]
  if (!is.null(z2)) {
    X2 <- make_X(model, make_ped_dat(data, term, z2))[, col_ind]
    X <- X - X2
  }
...
make_ped_dat <- function(x, term, z_vec) {
  ...
  ped_df[[LL_name]] <- outer(ped_df$tend, tz, FUN = ll_fun) *
    1L *
  ...
  if (length(t_mat_var) != 0) {
    ped_df[[t_mat_var]] <- matrix(
      unique(x[[t_mat_var]][, 1]),
      nrow = nrow(ped_df),
      ncol = nz
    )
  }
