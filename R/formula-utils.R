...
resolve_rhs_vars <- function(formula, data = NULL, exclude = character()) {
  rhs_vars <- get_rhs_vars(formula)
  if ("." %in% rhs_vars) {
    if (is.null(data)) {
      stop(
        "Cannot resolve `.` in the right-hand side without `data`.",
        call. = FALSE
      )
    }
    rhs_vars <- stats::terms(formula, data = data) %>%
      stats::delete.response() %>%
      all.vars()
  }

  setdiff(unique(rhs_vars), unique(c(exclude, ped_internal_vars())))
}
