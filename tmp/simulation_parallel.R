...
# ------------------------------------------------------------------------------
#
# BUILD FUNCTIONS FOR PARALLELIZATION
#
# ------------------------------------------------------------------------------

createData <- function(nData
                       , tra
                       ) {
  
  event.times <- rexp(nData, 0.9)
  f.cause <- rbinom(n, size = 1, prob = 1/3)
  f.cause <- ifelse(f.cause == 0, 2, 1)
  cens.times <- runif(nData,0,5)
  ...
  df <- data.frame(id, from, to, tstart = 0, time = obs.times, cens.times)
...
my.data <- createData(nData, tra)

set.seed(32168)
t <- runif(10,0,1)
test <- sapply(t, function(x) rexp(1, x))
