...
test_that("single-point latency grids produce finite lag-lead matrices", {
  ...
  ped <- as_ped(
    sim_df,
    Surv(time, status) ~
      x1 + cumulative(time, z.tz_single, tz_var = "tz_single"),
    cut = 0:5
  )
...
test_that("LL helpers and as_ped produce equivalent LL windows", {
  ...
  ped <- sim_df %>%
    as_ped(
      Surv(time, status) ~
        . +
          cumulative(time, z.tz1, tz_var = "tz1") +
          cumulative(
            time,
            z.tz2,
            tz_var = "tz2",
  ...
  LL1.2 <- get_laglead(ped) %>%
    filter(tz_var == "tz1") %>%
    filter(t != 0) %>%
    tidyr::spread(tz, LL) %>%
    select(-1:-2) %>%
    as.matrix()
  LL2.2 <- get_laglead(ped) %>%
    filter(tz_var == "tz2") %>%
    filter(t != 0) %>%
    tidyr::spread(tz, LL) %>%
    select(-1:-2) %>%
...
test_that("Cumulative effects are calculated correctly", {
    ...
    formula = ~ -3.5 +
      ...
      fcumu(
        ...
        f_xyz = function(t, tz, z)
          5 *
            (dnorm(t - tz, 4, 6) + dnorm(t - tz, 25, 4)) *
            z,
  ...
  ped <- as_ped(
    sim_df,
    Surv(time, status) ~
      x1 + x2 + cumulative(latency(tz3), z.tz3, tz_var = "tz3"),
    cut = 0:10
  )
  ...
  pam <- mgcv::gam(
    ped_status ~ s(tend) + x1 + s(x2) + s(tz3_latency, by = z.tz3),
    data = ped,
    family = poisson(),
    offset = offset
  )
  ...
  expect_equal(
    ndf$fit,
    c(.72, .88, 0.73, 0.46, 0.38, 0.26, 0.14),
    tolerance = 10e-3
  )
  ...
  cumu_eff <- get_cumu_eff(
    ped,
    pam,
    term = "z.tz3",
    z1 = seq(-1, 1, length.out = 5),
    z2 = 0
  )
