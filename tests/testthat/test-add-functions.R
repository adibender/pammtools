...
test_that("cumulative hazard functions work for PAM", {
  ...
  expect_data_frame(haz <- add_cumu_hazard(ped_info(ped), pam, ci=FALSE),
   nrows=5L, ncols=8L)
	expect_data_frame(haz <- add_cumu_hazard(ped_info(ped), pam), nrows=5L, ncols=10L)
	expect_error(add_cumu_hazard(haz, pam))
	expect_data_frame(add_cumu_hazard(haz, pam, overwrite = TRUE), nrows=5L, ncols=10L)

})

test_that("cumulative hazard functions work for PEM", {

  expect_data_frame(haz <- add_cumu_hazard(ped_info(ped), pem), nrows=5L, ncols=10L)
  expect_error(add_cumu_hazard(haz, pem))
  expect_data_frame(add_cumu_hazard(haz, pem, overwrite = TRUE), nrows=5L, ncols=10L)
...
test_that("survival probabilities functions work for PAM", {
  ...
  expect_data_frame(surv <- add_surv_prob(ped_info(ped), pam, ci = FALSE),
    nrows = 5L, ncols = 8L)
  expect_data_frame(surv <- add_surv_prob(ped_info(ped), pam), nrows=5L, ncols=10L)
  stest <- sapply(surv[, c("surv_prob", "surv_lower", "surv_upper")], function(z) {
    all(z >=0 & z <=1)
  })
  expect_identical(all(stest), TRUE)
  expect_error(add_surv_prob(surv, pam))
  expect_data_frame(add_surv_prob(surv, pam, overwrite = TRUE), nrows=5L, ncols=10L)
