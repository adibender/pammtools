...
pam <- mgcv::gam(
  ped_status ~ s(tend, k = 5) + complications,
  data = ped,
  family = poisson(),
  offset = offset
)
pam2 <- mgcv::gam(
  ped_status ~ s(tend, k = 5) + complications + s(age),
  data = ped,
  family = poisson(),
  offset = offset
)
...
pam3 <- mgcv::gam(
  ped_status ~
    s(tend, k = 5, by = as.factor(complications)) + as.factor(complications),
  data = ped,
  family = poisson(),
  offset = offset
)
...
test_that("hazard functions work for PAM", {
  ...
  expect_data_frame(
    haz <- add_hazard(ped_info(ped), pam),
    nrows = 5L,
    ncols = 11L
  )
  ...
  expect_data_frame(
    add_hazard(haz, pam, overwrite = TRUE),
    nrows = 5L,
    ncols = 11L
  )
  ...
  ## delta rule
  expect_data_frame(
    add_hazard(ped_info(ped), bam, ci_type = "delta"),
    nrows = 5L,
    ncols = 11L
  )
  ...
  expect_true(all(
    ndf2$cumu_hazard > ndf2$cumu_lower & ndf2$cumu_hazard < ndf2$cumu_upper
  ))
  expect_true(all(
    ndf3$cumu_hazard > ndf3$cumu_lower & ndf3$cumu_hazard < ndf3$cumu_upper
  ))
})
...
test_that("hazard functions work for PEM", {
  ...
  expect_data_frame(
    add_hazard(haz, pem, overwrite = TRUE),
    nrows = 5L,
    ncols = 11L
  )
})
...
test_that("cumulative hazard functions work for PEM", {
  ...
  expect_data_frame(
    add_cumu_hazard(haz, pem, overwrite = TRUE),
    nrows = 5L,
    ncols = 10L
  )
})
...
test_that("adding terms works for PEM", {
  expect_data_frame(
    term <- add_term(ped_info(ped), pem, term = "complications"),
    nrows = 5L,
    ncols = 10L
  )
  expect_error(
...
test_that("works for nonstandard baseline arguments", {
  ...
  p_pam <- mgcv::gam(
    ped_status ~ s(stop, k = 5) + complications,
    data = pseudonymous,
    family = poisson(),
    offset = offset
  )
  p_pem <- glm(
    ped_status ~ 0 + int + complications,
    data = pseudonymous,
    family = poisson(),
    offset = offset
  )
  ...
  expect_equal(
    add_cumu_hazard(
      dplyr::select(pseudonymous[1:5, ], -length),
      p_pam,
      time_var = "stop",
      interval_length = "length",
      ci = FALSE
  ...
  if (identical(attr(pseudo_ped, "time_var"), "tend"))
    attr(pseudo_ped, "time_var") <- "stop"
  expect_equal(
    get_cumu_coef(
      p_pam,
      pseudo_ped,
...
test_that("survival probabilities functions work for PAM", {
  ...
  stest <- sapply(
    surv[, c("surv_prob", "surv_lower", "surv_upper")],
    function(z) {
      all(z >= 0 & z <= 1)
    }
  )
...
test_that("CIF works with nonstandard time variable names", {
  ...
  ped_cr <- as_ped(df, Surv(time, status) ~ ., id = "id") %>%
    mutate(cause = as.factor(cause))
  pam <- gam(
    ped_status ~ s(stop, by = cause),
    data = rename(ped_cr, stop = tend),
    family = poisson(),
    offset = offset
  )
  ndf_stop <- ped_cr %>%
