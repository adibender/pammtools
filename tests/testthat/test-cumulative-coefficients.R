...
test_that("Cumulative coefficients work", {
  ...
  pam <- mgcv::gam(formula = ped_status ~ s(tend, by = as.ordered(complications)) +
      s(tend, by =age), data   = tumor_ped, family = poisson(), offset = offset)
  expect_warning(bam <- mgcv::bam(formula = ped_status ~ s(tend, by = as.ordered(complications)) +
      s(tend, by =age), data   = tumor_ped, family = poisson(), offset = offset,
      method = "REML"))
  # summary(pamm)
  # plot(pamm, page=1)
  cumu_coef_bam <- get_cumu_coef(bam, tumor_ped, terms=c("age", "complications"))
