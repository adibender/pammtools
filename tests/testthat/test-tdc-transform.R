...
test_that("Concurrent TDC are transformed correctly", {
  ...
  expect_equal(
    unique(ped$tend)[1:7],
    c(c(176, 182, 192, 364) + 10, 365 + 10, 400, 743 + 10))
  expect_equal(ped$bili,
               c(rep(14.5, 3), rep(21.3, 3), rep(1.1, 2), rep(0.8, 3), rep(1, 3),
                 rep(1.9, 2), 2.6, 3.6, 4.2, 3.6, 1.4, rep(1.1, 3), rep(1.5, 3),
                 rep(1.8, 2)))
  ...
  ped <- as_ped(
    ...
    id = "id")
  expect_data_frame(ped, nrow = 50, ncol = 11)
  expect_equal(sum(ped$ped_status), 2)
  expect_equal(sort(unique(ped$tend)), sort(unique(c(t[-3], tz, tz+10))))
  expect_equal(ped$bili,
               c(rep(14.5, 5), rep(21.3, 5), rep(1.1, 4), rep(0.8, 5), rep(1, 5),
                 rep(1.9, 3), rep(2.6, 2), rep(3.6, 2), rep(4.2, 2), rep(3.6,2),
                 rep(1.4, 3), rep(1.1, 5), rep(1.5, 4),
                 rep(1.8, 3)))
  expect_equal(ped$protime,
               c(rep(12.2, 4), rep(11.2, 6), rep(10.6, 2), rep(11, 5),
                 rep(11.6, 6), rep(10.6, 3), rep(11.3, 2), rep(11.5, 7),
                 rep(12, 11), rep(13.3, 4)))
# when maxtime is set
  ped <- as_ped(
    data    = list(event_df, tdc_df),
    formula = Surv(time, status)~. | concurrent(bili, protime, tz_var = "day"),
