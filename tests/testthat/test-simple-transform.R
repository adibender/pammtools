...
test_that("Transformation of regular survival data works", {
  ...
  ped_vet <- split_data(
    data = tumor,
    Surv(days, status) ~ complications + age,
    cut = c(0, 100, 400),
    id = "ID"
  )
  ...
  expect_data_frame(
    ped <- tumor %>%
      as_ped(Surv(days, status) ~ complications + age, cut = c(0, 100, 400)),
    nrows = 12L,
    ncols = 8L
  )
  expect_is(ped, "ped")
  expect_subset(
    c("ped_status", "tstart", "tend", "interval", "offset"),
    names(ped)
  )
  ...
  expect_data_frame(
    tumor %>% as_ped(Surv(days, status) ~ complications, cut = c(0, 100, 400)),
    nrows = 12L,
    ncols = 7L
  )
  ...
  tumor$id2 <- tumor$id
  expect_data_frame(
    ped <- tumor %>% as_ped(Surv(days, status) ~ ., cut = c(0, 100, 400)),
    nrows = 12L,
    ncols = 14L
  )
  ...
  expect_data_frame(
    ped <- tumor %>%
      as_ped(Surv(days, status) ~ ., cut = c(0, 100, 400), id = "id2"),
    nrows = 12L,
    ncols = 13L
  )
