...
using algorithms designed to fit Generalized Linear Models. This approach yields equivalent coefficient estimates as Cox Proportional Hazards models if (1) there are no ties (i.e., simultaneous events) in the data
and (2) all unique event times are used as interval cut points in order to transform the data into 
the format suitable for PEMs (see `?split_data` and `vignette("data-transformation", package = "pam")`).
...
We first demonstrate the equivalence using Freireichs Leukemia data [@freireich1963] 
provided in the `bpcp` package.
...
- PAM estimation scales better when the number of intervals becomes very large: 
	PEMs need to estimate one parameter per interval for the baseline hazard, while the number 
	of parameters in PAMs only depends on the number of basis functions used for the spline estimate of 
	the baseline hazard.

- In PEMs, baseline hazard estimates for each interval can vary a lot between neighboring 
	intervals, especially when intervals only contain few events. For 
	PAMs on the other hand estimates of the baseline hazard in neighboring 
	intervals are similar due to penalization unless the data provides very strong evidence for 
	large changes between neighboring intervals. 

Note that coefficient estimates in PAMs are no longer equivalent to those from Cox PH models, since 
the estimation of the baseline hazard is performed semi-parametrically. In our 
experience the differences are not very large. Using the example above we get:

```{r}
## compare to PAM 
pam.treat <- gam(ped_status ~ s(tend) + treatment, data = leuku.ped, 
	family = "poisson", offset = offset)
coef(pam.treat)["treatmentplacebo"]
coef(pem.treat)["treatmentplacebo"]
coef(cph.treat)["treatmentplacebo"]
```
