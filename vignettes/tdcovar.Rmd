...
recidivism_long %<>% 
  mutate(employed_lag1 = lag(employed, default=0)) %>% 
...
cph <- coxph(Surv(start, stop, arrest)~ fin + pspline(age) + race + wexp + mar + 
  paro + pspline(prio) + employed_lag1,
...
## (length.out=100)
age_df <- recidivism_long %>% make_newdata(expand="age", length.out=100)
...
pinf_df <- recidivism_long %>% make_newdata(expand="prio", length.out=100)
pinf_df %<>% add_term(pam, term="prio") %>% 
  mutate(cphfit = predict(object=cph, ., type="terms")[,7])

## plot of the prio effect 
prio_gg <- smooth_gg %+% pinf_df + aes(x=prio)
```
...
bili_df <- pbc_ped %>% ungroup() %>% 
  make_newdata(expand="bili", length.out=100) %>%
...
protime_df <- pbc_ped %>% ungroup() %>% 
  make_newdata(expand="protime", length.out=100) %>% 
