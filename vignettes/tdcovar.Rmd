...
using the prominent recidivism data discussed in detail in @Fox2011. 
...
splitting at each week that subjects are in the risk set):
...
This demonstrates, that results can differ substantially, when only the 
baseline values of TDCs are used in anlysis. 
...
The `split_tdc` function performs the following tasks: 
  ...
  - extract unique event times and measurement times of the TDCs
  - split the follow-up according to these times (essentially calling 
  `split_data` with above cut-points on `event_df` after removing TDCs)
  - merge the expanded data set with the data set containing information 
  on TDCs by ID and time
  - fill the values of TDCs for time-points that did not occur in `tdc_df` by 
  carriing last value forward. 
...
Coeffiicent estimates are very similar for both models, especially for the 
