...
Similarl to other applications of `add_*` functions, we can additionally group by other covariate values, e.g.:

```{r fig.width = 12, fig.height = 5, out.width = "600px"}
ndf <- ped_stacked %>%
  make_newdata(tend = unique(tend), cause = unique(cause), sex = unique(sex))
class(ndf) <- c("ped", "data.frame")
ndf <- ndf %>%
  group_by(cause, sex) %>%
  add_cif(pam_csh_shared)
ggplot(ndf, aes(x = tend, y = cif)) +
