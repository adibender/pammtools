...
gg.bz + geom_step(aes(col=strata))
```
...
```{r, fig.width=5, fig.height=4, dependson="ggbase", warning=FALSE}
pinf <- ped %>% 
  group_by(celltype) %>% 
  ped_info() %>% 
  add_cumhazard(pam) %>% 
...
including the treatment (`trt`), age (`age`) and the Karnofsky Score (`karno`), 
we get the model 
