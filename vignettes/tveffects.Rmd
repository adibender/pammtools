...
karno_df$fte <- predict(pam3, newdata=karno_df, type="terms")[,"te(tend,karno)"]
karno_gg <- ggplot(karno_df, aes(x=tend, y=fte)) +
  geom_step(aes(col=factor(karno))) +
  scale_color_brewer(name=expression(x[plain(karno)]), palette = "Blues") +
...
time_df  <- combine_df(
  ...
  # caution: only use values for tend that were actually used as interval end 
  # points to avoid interpolating piece-wise constant time-varying effects 
  # between steps!
  data.frame(tend  = c(1, 51, 200, 314)))
time_df$fte <- predict(pam3, newdata=time_df, type="terms")[,"te(tend,karno)"]
time_gg <- ggplot(time_df, aes(x=karno, y=fte)) +
  geom_line(aes(col=factor(tend))) +
  scale_color_brewer(name="t", palette="Reds") +
  ylab(expression(f(list(x[plain(karno)],t)))) +
