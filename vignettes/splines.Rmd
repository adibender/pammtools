...
veteran %<>% filter(time < 400) %>% mutate(prior = 1*(prior==10))
head(veteran)
```
...
$$
\lambda(t|x) = \lambda_0(t)\exp(f(\text{age}})) 
$$
and 
$$
\lambda(t|x) = \exp(f_0(t_j) + f(\text{age}))
$$
respectively: 
...
age_df <- ped %>% make_newdata(expand = "karno", length.out = 100) %>% 
  add_term(pam, term="karno") %>% 
  mutate(cox = predict(cph, newdata=., type="terms")[, "pspline(karno, df = 0)"] -
           predict(cph, newdata=data.frame(karno=mean(veteran$karno))))
ggplot(age_df, aes(x=karno, ymin=low, ymax=high)) + 
  geom_line(aes(y=fit, col="PAM")) + 
  geom_ribbon(alpha=0.2) + 
  geom_line(aes(y=cox, col="Cox PH"), lty=2)+
...
time-varying terms of the form $f(t)\cdot x_{karno}$ and $f(x_karno, t)$ see 
...
plot(mpam, select = 2, se = TRUE, shift = const, 
  ylim = c(-0.5-const, 2-const), main="Fit under monotonicity constraint")
```
