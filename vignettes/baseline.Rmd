...
tdf <- data.frame(time = rweibull(100, 1.5, 2)) %>% 
  mutate(
    time  = round(time, 3),
    status = 1*(time > rweibull(100, 1.5, 2)))
head(tdf)
```
...
base_df <- basehaz(coxph(Surv(time, status)~1, data = tdf))
...
In the figure below we compare the previous baseline estimates with the estimates
from a correctly specified model (Weibull-AFT). All models are in good 
agreement.
...
int_df$aft <- exp(-coef(aft))^(1/aft$scale) * int_df$tend^(1/aft$scale)
