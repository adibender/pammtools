...
dplyr::filter(ped, id %in% c(42, 85, 112))
```
...
the subject "visited". Thus subject `42` was under risk during 5 intervals, 
subject `85` was under risk during one interval, and subject `112` in 
33 intervals. In addition to the optional `id` variable the function also 
creates 
...
ped2 <- split_data(Surv(time, status)~., data = veteran, cut = c(0, 50, 200, 400), 
  id = "id")
ped2 %>% filter(id %in% c(42, 85, 112))
```
Note that now have subjects `42` and `85` have only one row in the data set. 
