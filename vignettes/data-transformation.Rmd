...
handled by the `split_data` function. This function internally calls
...
Using the `pammtools` package this is easily achieved with the `split_data` function
as follows:
...
Here
  ...
  *partial effects* of the TDC $z(t_e)$ observed at exposure time
  ...
  $\mathcal{T}(t)$. The most common definition is
...
Note that
  ...
  thus for $\mathcal{T}_1$ the lag-lead window must not be explicitly specified.
  ...
  when multiple covariates should have the same time components and the
...
the exposure time provided to `te_var` will be used as a suffix to the new
matrix columns to indicate with respect to what exposure the columns had been
constructed, however, you can override the suffix by specifying the
