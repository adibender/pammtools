...
terms with different `tz_var` arguments can be specified. If different
`tz_var` arguments are provided, the union of both will be used as cut points
...
ped_sim <- as_ped(
  data = simdf_elra,
  formula = Surv(time, status)~.| cumulative(time, latency(tz1), z.tz1, tz_var="tz1") +
...
```{r}
ped_sim %>% select(id, LL_tz1) %>% head(16)
ped_sim %>% select(id, LL_tz2) %>% head(16)
```
