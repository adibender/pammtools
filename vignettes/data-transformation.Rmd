...
terms with different `te_var` arguments can be specified. If different
`te_var` arguments are provided, the union of both will be used as cut points
in addition to usual cut points.
...
|------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| $\int_{\mathcal{T}_1}h(t-t_e, z_1(t_e))$                                            | `cumulative(latency(te1), z1, te_var= "te1")`                                              |
| $\int_{\mathcal{T}_1}h(t, t-t_e, z_1(t_e))$                                        | `cumulative(time, latency(te1), z1, te_var="te1")`                                         |
| $\int_{\mathcal{T}_1}h(t, t_e, z_1(t_e))$                                          | `cumulative(time, te1, z1, te_var="te1")`                                                  |
| $\int_{\mathcal{T}_1}h(t, t_e, z_1(t_e)) + \int_{\mathcal{T}_2}h(t-s_e, z_2(s_e))$ | `cumulative(time, te1, z1, te_var="te1") + cumulative(latency(te2), z2, te_var="te2", ll_fun=ll2)` |
| ...                                                                                | ...                                                                                     |
...
Note that
  ...
  - `cumulative` makes no difference between partial effects such as $h(t-t_e, z(t_e))$
  ...
  - To tell `cumulative` which of the variables specified is the exposure time $t_e$,
  the `te_var` argument must be specified within each `cumulative` term. The
...
ped <- as_ped(
  ...
  formula = Surv(survhosp, PatientDied) ~ . |
    cumulative(latency(Study_Day), caloriesPercentage, te_var="Study_Day"),
  cut     = 0:30,
  id = "CombinedID")
str(ped, 1)
```
...
$t-t_e$ occurs in both partial effects, we only need to specify one `cumulative` term.
...
ped_sim <- as_ped(
  data = simdf_elra,
  formula = Surv(time, status)~.| cumulative(time, latency(te1), z.te1, te_var="te1") +
