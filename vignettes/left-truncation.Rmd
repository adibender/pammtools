...
library(ggplot2)
theme_set(theme_bw())
has_eha <- requireNamespace("eha", quietly = TRUE)
```

```{r, echo = FALSE, results = "asis"}
if (!has_eha) {
  cat(
    "This vignette example requires the optional package `eha` and is skipped ",
...
pam <- pamm(ped_status ~ s(tend) + mother, data = infants_ped)
...
```{r, fig.width = 4, fig.height = 4, out.width = "300px", eval = has_eha}
