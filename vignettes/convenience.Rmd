...
# use only a faction of observations for convenience
set.seed(23032017)
ped <- split_data(Surv(futime, death)~age + hgb + sex, data=mgus2, id="id")
pam <- gam(status~s(tend) + age + s(hgb), data=ped, family=poisson(), offset=offset)
```
...
Here the receiving treatment increases the risk of dying by a factor of 
$\exp(\Sexpr{round(coef(pam.treat)[2], 2)}) = \Sexpr{round(exp(coef(pam.treat)[2]), 2)}.
...
i.e. the part that controls the shape of the $\log$-baseline-hazard. 
...
be maintained throughout, for example, if we wanted to plot the cumulative hazard 
for different sexes 

```{r}
pam.sex <- gam(status~s(tend) + sex + age + s(hgb), data=ped, family=poisson(), offset=offset)
```

```{r}
pinfo.sex <- ped %>% group_by(sex) %>% ped_info()
pinfo.sex <- pinfo.sex[, -9]
pinfo.sex %<>% add_cumhazard(pam)
ggplot(pinfo.sex, aes(x=tend, y=cumhazard, ymin=cumlower, ymax=cumupper)) + 
	geom_step(aes(col=sex)) + 
	geom_stepribbon(aes(fill=sex), alpha=0.2)
```
