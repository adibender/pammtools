...
```{r}
ped_info %<>% add_hazard(pam)
ped_info %>% select(interval, hazard, lower, upper) %>% head()
ped_info %>% select(interval, hazard, lower, upper) %>% tail()
```

Which in turn can be conveniently plotted using respective functions, e.g. 

```{r}
ggplot(ped_info, aes(x = tend)) + 
...
```{r}
gg.chaz <- ped_info %>% add_cumhazard(pam) %>% 
...
```{r}
gg.surv <- ped_info %>% add_survprob(pam) %>% 
