...
has_tbfmultinomial <- requireNamespace("TBFmultinomial", quietly = TRUE)
```
...
if (!has_tbfmultinomial) {
  cat(
    "The worked example below requires the optional package ",
    "`TBFmultinomial` and is skipped when that package is not installed.\n\n",
    sep = ""
  )
}
```
...
```{r, warning = FALSE, results = "hide", eval = has_tbfmultinomial}
...
```{r, eval = has_tbfmultinomial}
event_df %>% head()
tdc_df %>% head()
```

```{r fig.width = 6, fig.height = 4, echo = FALSE, out.width = "600px", fig.cap = "SOFA profiles for individual patients. Each line represents a patient, each pixel represents a measurement.", eval = has_tbfmultinomial}
tdc_df %>% group_by(ID) %>%
  mutate(
...
```{r, message = FALSE, eval = has_tbfmultinomial}
ped <- as_ped(
...
```{r, output.lines = c(1:4, 10:13), eval = has_tbfmultinomial}
str(ped, 1)
```

```{r, echo = FALSE, fig.width = 4, fig.height = 4, out.width = "400px", fig.cap = "Lag-lead window for the first 10 days of the follow-up.", eval = has_tbfmultinomial}
...
```{r fig.width = 9, fig.height = 4, warning = FALSE, out.width="650px", eval = has_tbfmultinomial}
...
```{r fig.width = 4, fig.height = 4, out.width = "400px", eval = has_tbfmultinomial}
...
```{r, fig.width = 8, fig.height = 4, out.width = "650px", eval = has_tbfmultinomial}
