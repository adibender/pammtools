...
be comptuted similar to any other model (see respective help pages `?pec::pec` and
`?pec::cindex`). Note that for thechnical reasons, evaluation of `pamm` objects
...
The results illustrate that one model is not necessarily better w.r.t. the prediction error for all time-points. For example `pam3` is better than `pam2`
in the beginning and worse towards the end. Similarly, the integrated brier score
(IBS) olso depends on the evaluation time.
...
### C-Index
Examplary, we calculate the C-Index, hower, note the warning message and the
cited literature.
```{r}
cindex(
  list(pam1 = pam1, pam2 = pam2, pam3 = pam3),
