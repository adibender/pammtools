...
sim_x_compl <- rbind(x1_df)
sim_t_compl <- rbind(tend_df)
...
gg_x1 <- ggplot(sim_x_compl_new, aes(x = x1)) +
  ...
  geom_line(aes(y = mean_fit), color="firebrick2", lwd = 1.1) +
...
sim_t_compl_sub <- sim_t_compl_new %>% filter(transition == "1->2")
